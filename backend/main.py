...
async def llm_status():
    """
    Check which LLM is configured and if it's working
    """
    import os
    ...
    try:
        # Try to initialize Groq client (or key)
        if init_groq_client():
            ...
            try:
                ...
                if response_text:
                    ...
                    status["error"] = "No response from Groq"
            except Exception as e:
                status["error"] = f"Groq API call failed: {str(e)}"
        else:
            status["error"] = "GROQ_API_KEY not configured in environment"
    except Exception as e:
        status["error"] = f"Failed to initialize Groq client: {str(e)}"
...
@app.post("/projects")
async def create_project(request: dict = Body(...)):
    """Create new project"""
...
async def update_project_settings(project_name: str, request: dict = Body(...)):
...
async def scan_file_with_context(file_path: str, context: str):
    ...
    try:
        ...
        from engines.governance_engine_api import run_governance_check_with_context
...
async def confirm_deployment(request: dict = Body(...)):
    """
...
async def scan_code(file: UploadFile = File(...)):
    """
    Endpoint to trigger code scan - Accepts uploaded file directly
    Similar to Flask implementation: reads file content and analyzes it
    """
    try:
        # Read uploaded file content - same way as Flask
        try:
            ...
            return JSONResponse(
                status_code=400,
                content={"error": f"File is not a valid text file or encoding issue: {str(e)}"}
            )
        
        # Save to temporary file for analysis engines that need file paths
...
async def github_webhook(request: dict = Body(...)):
    """
...
async def verify_github_signature(request: dict = Body(...), signature: str = ""):
    """
...
async def add_suggestion_comment(file: UploadFile = File(...), line: int = None, suggestion: str = None):
    ...
    try:
        ...
        if 0 < line <= len(lines):
            ...
            print(f"✅ Added suggestion comment to {file.filename} at line {line}")
            ...
            return {"status": "error", "message": f"Line {line} out of range (file has {len(lines)} lines)"}
    except Exception as e:
...
async def apply_fix(file: UploadFile = File(...), line: int = None, new_code: str = None):
    """
    Apply auto-remediation to uploaded file
    """
    try:
        # Read uploaded file
        content = await file.read()
        ...
        if line and new_code:
            return await apply_single_line_fix_to_content(file_content, line, new_code, file.filename)
        else:
            return {"status": "error", "message": "Missing line number or new code"}
    except Exception as e:
        return JSONResponse(status_code=400, content={"error": str(e)})

async def apply_single_line_fix_to_content(file_content: str, line_num: int, new_code: str, filename: str):
    """
    Apply a single line fix to file content
    """
    try:
        lines = file_content.split('\n')
        ...
        if 0 < line_num <= len(lines):
            ...
            if new_code.strip():
                ...
                print(f"✅ Applied fix to {filename} at line {line_num}")
...
async def critical_alert(request: dict = Body(...)):
    """
